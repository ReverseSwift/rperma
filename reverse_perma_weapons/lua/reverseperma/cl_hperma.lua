		...
		content = function(frame, ent, purchases)
			...
			for _, v in pairs(self.config.categories) do
				...
				category.Paint = function(s, w, h)
					...
					draw.SimpleText(v[1], "RPermaTitle", 10, h / 2, Color(220, 220, 220), 0, 1)
				end
				...
				for k2, v2 in pairs(rperma.config.weapons) do
					if v2.category == v[1] then
						...
						item.Paint = function(s, w, h)
							...
							draw.SimpleText(wepinfo.PrintName, "RPermaTitle", 10, 5, Color(220, 220, 220))
							draw.SimpleText(DarkRP.formatMoney(v2.price), "RPermaPrice", 10, 30, LocalPlayer():canAfford(v2.price) and Color(0, 195, 165) or Color(255, 50, 50))
						end
						...
						if not purchases[k2] then
							...
							btn.Paint = function(s, w, h)
								...
								draw.SimpleText(purchases[k2].equipped == 0 and "Equip" or "Unequip", "RPermaBTN", w/2, h/2, Color(220, 220, 220), 1, 1)
							end
							btn.DoClick = function()
								net.Start("rperma_equip")
									net.WriteEntity(ent)
									net.WriteString(k2)
			...
			if table.Count(purchases) > 0 then
				...
				for k, v in pairs(purchases) do
					...
					item.Paint = function(s, w, h)
						...
						draw.SimpleText("DURABILITY: "..tostring(v.durability).." / 100", "RPermaSmall", w / 2, 29, Color(220, 220, 220), 1)
						draw.SimpleText(wepinfo.PrintName, "RPermaTitle", 10, 5, Color(220, 220, 220))
					end
					...
					btn.Paint = function(s, w, h)
						...
						draw.SimpleText(purchases[k].equipped == 0 and "Equip" or "Unequip", "RPermaBTN", w/2, h/2, Color(220, 220, 220), 1, 1)
					end
					btn.DoClick = function()
						net.Start("rperma_equip")
							net.WriteEntity(ent)
				...
				notings.Paint = function(s, w, h)
					draw.SimpleText("You don't own any items!", "RPermaLarge", w / 2, h / 2, Color(220, 220, 220), 1, 1)
				end
			end
...
function rperma:OpenMenu(ply, ent, purchases)
	XeninUI:CreateFont("RPermaLarge", 30)
	XeninUI:CreateFont("RPermaTitle", 20)
	XeninUI:CreateFont("RPermaPrice", 18)
	XeninUI:CreateFont("RPermaBTN", 18)
	XeninUI:CreateFont("RPermaSmall", 14)
	...
	net.Receive("rperma_success", function()
		frame:Remove()
	end)
end

net.Receive("rperma_openmenu", function()
	rperma:OpenMenu(LocalPlayer(), net.ReadEntity(), net.ReadTable())
end)
